...
// Turn off SwiftVerifyEmittedModuleInterface for the PostHog pod.
// Xcode 26+ runs a strict module-interface verifier on release builds
// (SwiftVerifyEmittedModuleInterface). The underlying `PostHog` iOS
// SDK — pulled in as a pod dependency of `posthog-react-native` —
// emits an interface that references itself (`import PostHog`), which
// the verifier rejects with "underlying Objective-C module 'PostHog'
// not found" because PostHog ships without an ObjC modulemap.
//
// The PostHog SDK bumps its patch version on CocoaPods trunk without
// our lockfile catching it (ios/Podfile.lock isn't tracked in git —
// the whole ios/ tree is regenerated by prebuild), so any CI run can
// silently pick up a new PostHog point release. This flip disables
// the verifier for that one pod so a bad emit from any PostHog
// version stops breaking release archives.
//
// Verified failure signature it addresses (CI log from a v356staging
// release archive):
//   error: underlying Objective-C module 'PostHog' not found
//   error: failed to verify module interface of 'PostHog' due to the
//          errors above; the textual interface may be broken by
//          project issues or a compiler bug
//   ** ARCHIVE FAILED **
//
// Same run's context: 'Installing PostHog (3.68.4)' — an older
// build 20h earlier picked up 3.68.2 and archived cleanly.
//
// Only PostHog is targeted so the verifier stays on for our own
// code + other pods where it does useful work.
