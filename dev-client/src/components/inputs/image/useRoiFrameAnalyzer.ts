...
// Colour codes match RoiOverlay's expectations:
export const COLOR_UNKNOWN = -1;
export const COLOR_RED = 0;
export const COLOR_GREEN = 1;

// Variance threshold below which a ROI is "consistent" (green).
// Matches the Android tuning (see CameraSessionManager.kt). Real-world
// variance is often <100 for a well-lit uniform card, >1000 for mixed
// edges.
const VARIANCE_GREEN_THRESHOLD = 200;
...
export function useRoiFrameAnalyzer(): UseRoiFrameAnalyzerResult {
  const refCode = useSharedValue(COLOR_UNKNOWN);
  const sampleCode = useSharedValue(COLOR_UNKNOWN);
  ...
  const frameOutput = useFrameOutput({
    ...
    onFrame: frame => {
      ...
      try {
        ...
        refCode.value =
          refStats.variance < VARIANCE_GREEN_THRESHOLD
            ? COLOR_GREEN
            : COLOR_RED;
        sampleCode.value =
          sampleStats.variance < VARIANCE_GREEN_THRESHOLD
            ? COLOR_GREEN
            : COLOR_RED;
          ...
          console.log(
            `[phase8.4] frame ${w}x${h} orient=${frame.orientation} ` +
              `refSensor=(${Math.round(refSensor.x * w)},${Math.round(
                ...
                1,
              )} refVar=${refStats.variance.toFixed(1)} ` +
              `sampleSensor=(${Math.round(sampleSensor.x * w)},${Math.round(
                ...
                1,
              )} sampleVar=${sampleStats.variance.toFixed(1)}`,
          );
        }
      } finally {
        frame.dispose();
  ...
  return {frameOutput, refCode, sampleCode};
}
